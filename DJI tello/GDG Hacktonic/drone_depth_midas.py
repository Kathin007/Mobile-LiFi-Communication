"""
drone_depth_midas_ui_fast.py
Lightweight Monocular Depth Estimation (MiDaS) + Manual Drone Control (Pygame)
- No YOLO (max performance)
- HUD: battery, height, status, FPS, model, movement, signal quality
- Smooth controls with higher SPEED (for real-time flight feeling)
"""
...
import KeyPressModule as kpm   # your existing pygame key handler
...
FRAME_SKIP = 2
DEPTH_BUFFER = 5
SPEED = 60                   # Faster manual movement like main_control.py
...
# ---------------- LOAD MiDaS ----------------
print("Loading MiDaS_small for real-time depth...")
...
# ---------------- CAMERA THREAD ----------------
def camera_reader():
    global tello, frame_ok_counter, frame_drop_counter
    try:
        tello = Tello()
        tello.connect()
        ...
        print("⚠️ Tello connection failed:", e)
        stop_event.set()
        return
...
# ---------------- DEPTH THREAD ----------------
def depth_processor():
    global cam_fps_display, _cam_fps_time, _cam_frame_count, _infer_fps
    frame_cnt = 0
    while not stop_event.is_set():
        if frame_q.empty():
            time.sleep(0.002)
        ...
        _infer_fps = 1.0 / (time.time() - t0 + 1e-6)
...
def main():
    ...
    cv2.namedWindow("MonoSight — Depth Only (HUD)", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("MonoSight — Depth Only (HUD)", WIDTH*2, HEIGHT + HUD_HEIGHT)
    ...
    while not stop_event.is_set():
        ...
        draw_transparent_rect(combined, (8, 8, 380, 86), HUD_BG_COLOR, HUD_ALPHA)
        ...
        hud_x, hud_y = 16, 28
        t1 = f"Battery: {telemetry.get('battery',0)}% | Height: {telemetry.get('height',0)}cm"
        t2 = f"Status: {telemetry.get('status','Idle')} | FPS: {int(cam_fps_display):d}/{int(_infer_fps):d}"
        t3 = f"Movement: {move_text}"
        t4 = f"Signal: {int(quality*100)}% | Model: MiDaS_small"

        hud_text(combined, t1, (hud_x, hud_y), 0.58, (220,220,220), 1)
        hud_text(combined, t2, (hud_x, hud_y+22), 0.50, (200,200,200), 1)
        hud_text(combined, t3, (hud_x, hud_y+44), 0.50, (190,230,180), 1)
        hud_text(combined, t4, (hud_x, hud_y+66), 0.45, (160,160,255), 1)
        ...
        cv2.imshow("MonoSight — Depth Only (HUD)", combined)
